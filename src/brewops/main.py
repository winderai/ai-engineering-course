...
from brewops.information import (
...
from brewops.agent import agent_router, initialize_agent, AgentConfig
...
# Initialize information services
product_service = ProductInformationService(
    data_directory=os.getenv("DATA_DIRECTORY", "./data")
)
order_service = OrderQueryService(data_directory=os.getenv("DATA_DIRECTORY", "./data"))
...
async def startup_event():
    ...
    logger.info("Initializing information services...")

    # Initialize product service
    try:
        products = product_service.load_products()
        if products:
            product_service.index_products(products)
            logger.info(f"Loaded and indexed {len(products)} products")
        else:
            logger.warning("No products found to load")
    except Exception as e:
        logger.error(f"Error initializing product service: {e}")

    # Initialize order service
    try:
        order_service.initialize_database()
        order_service.load_orders_from_csv()
...
@app.post("/information/products/search", response_model=ProductSearchResponse)
async def search_products(request: ProductSearchRequest) -> ProductSearchResponse:
    """Search products using natural language RAG pipeline."""
    try:
        results = product_service.search_products_rag(
...
@app.get("/information/products/{product_id}", response_model=ProductInfo)
async def get_product_by_id(product_id: str) -> ProductInfo:
...
@app.get("/information/products/", response_model=List[ProductInfo])
async def list_products(limit: int = 50, offset: int = 0) -> List[ProductInfo]:
    """List all available products."""
    all_products = list(product_service.products.values())
    return all_products[offset : offset + limit]


@app.post("/information/orders/query", response_model=OrderQueryResponse)
async def query_orders(request: OrderQueryRequest) -> OrderQueryResponse:
    """Query orders using natural language with LLM-powered SQL generation."""
    try:
        start_time = time.time()

        # Generate SQL from natural language
        schema_context = order_service.get_schema_context()
        sql_query = order_service.generate_sql_with_llm(
            request.question, schema_context
        )

        if not sql_query:
            raise HTTPException(
                status_code=400, detail="Could not generate SQL query from question"
            )

        # Execute query
        orders = order_service.execute_order_query(sql_query)
        ...
        raise HTTPException(status_code=500, detail=f"Order query failed: {str(e)}")


@app.get("/information/orders/{order_id}")
...
@app.get("/information/health", response_model=HealthResponse)
async def get_system_health() -> HealthResponse:
    """System health check and component status."""
    try:
        # Check RAG pipeline
        rag_ready = (
            len(product_service.products) > 0
            and product_service.vector_store.embeddings_matrix is not None
        )

        # Check DuckDB connection
        duckdb_active = order_service.db_connection is not None

        # Count indexed items
        products_indexed = len(product_service.products)

        # Count orders (try to query)
        orders_loaded = 0
        try:
            if duckdb_active and order_service.db_connection is not None:
                result = order_service.db_connection.execute(
                    "SELECT COUNT(*) FROM orders"
                ).fetchone()
                orders_loaded = result[0] if result else 0
        except Exception:
            pass

        uptime = time.time() - startup_time

        status = "healthy" if rag_ready and duckdb_active else "degraded"

        return HealthResponse(
            status=status,
            rag_pipeline_ready=rag_ready,
            duckdb_connection_active=duckdb_active,
...
@app.post("/information/products/reindex")
